...
// --------------------- the main menu ---------------------
pub var MainMenu:df.MBAR = buildMenuBar(.{
    ...
    .{"~File", df.PrepFileMenu, "Read/write/print files. Go to DOS", -1, .{
            ...
            .{df.SEPCHAR,    0,            0,     0,     null      },
            .{"~Print",      df.ID_PRINT,  0,     df.INACTIVE,     "ID_PRINT"  },
            .{"P~rinter setup...",    df.ID_PRINTSETUP,   0,     0,     "ID_PRINTSETUP"  },
            .{df.SEPCHAR,    0,            0,     0,     null      },
...
fn buildMenu(comptime pulldowns:anytype) [df.MAXPULLDOWNS+1]df.MENU {
    var result:[df.MAXPULLDOWNS+1]df.MENU = undefined;

    inline for(0..(df.MAXPULLDOWNS+1)) |idx| {
        result[idx] = .{
            .Title = null,
            .Selection = 0,
        };
...
fn buildPopDown(comptime popdowns:anytype) [df.MAXSELECTIONS+1]df.PopDown {
    var result:[df.MAXSELECTIONS+1]df.PopDown = undefined;

    inline for(0..(df.MAXSELECTIONS+1)) |idx| {
        result[idx] = .{
            .SelectionTitle = null,
            .ActionId = 0,
            .Accelerator = 0,
